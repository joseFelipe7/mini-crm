...
class Collaborator extends REST_Controller {
   public function index_get(){
      ...
      if (!$this->pagination_custom->validSort($sort, ['name', 'email'])){
      ...
      if (!$this->pagination_custom->validFilter($filter, ['name', 'email'])){
      ...
      $collaborator =  $this->collaborator->list($page, $perPage, $sort, $filter);
      $totalCollaborator =  $this->collaborator->listTotal($filter);
      
      $meta =  $this->pagination_custom->transformMeta($page, $perPage, $totalCollaborator);
